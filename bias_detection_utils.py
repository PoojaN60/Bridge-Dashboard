...
def show_bias_visualizations(test_df):
    ...
    pivot_data = pd.pivot_table(
        biased_reviews_per_dept, values='predicted_bias_flag',
        index='department', columns='manager_name',
        aggfunc='count', fill_value=0
    )
    st.write("### Heatmap: Bias Distribution")
    st.pyplot(sns.heatmap(pivot_data, annot=True, cmap='Blues', fmt='d', linewidths=0.5).figure)
